...
export const KeyModal: React.FC<KeyModalProps> = ({ show, onClose, vipTab, redeemKey, userEmail }) => {
  ...
  return (
    <div className="fixed inset-0 bg-black/80 backdrop-blur-sm flex items-center justify-center p-4 z-[70] font-sans animate-in zoom-in-95 duration-200 overflow-y-auto">
      <div className="bg-card border-border border-2 border border-[#3B82F6]/25 p-6 sm:p-8 max-w-lg w-full relative overflow-hidden my-8 brut-card rounded-3xl shadow-2xl">
        <div className="absolute top-0 right-0 w-64 h-64 bg-primary/10 rounded-full blur-3xl pointer-events-none"></div>

        <div className="text-center mb-6 relative z-10">
            <div className="w-16 h-16 bg-primary text-primary-foreground flex items-center justify-center mx-auto mb-4 border border-[#3B82F6]/20 rounded-2xl">
            ...
            <a href="https://discord.gg/yourlink" target="_blank" rel="noopener noreferrer" className="mt-4 inline-flex items-center gap-2 px-6 py-2.5 bg-card hover:bg-[#4752C4] text-white text-sm font-bold transition-all hover:scale-105 brut-card rounded-xl">
          ...
          {vipTab === 'key' && (
              ...
              <div>
                  ...
                  <div className="relative">
                    ...
                    <input required name="key" type="text" className="w-full bg-card border border-border border-2 py-3 pl-10 pr-4 text-sm focus:border-[#3B82F6] outline-none text-white transition-all placeholder:text-zinc-400 brut-card rounded-xl" placeholder="DEV-XXXXX-XXXXX" />
                  </div>
              </div>
              <button type="submit" className="w-full bg-blue-600 hover:bg-blue-500 text-white font-bold py-3.5 transition-all mt-4 rounded-xl shadow-lg">
                เปิดใช้งาน
              </button>
            </form>
          )}
        ...
        <button onClick={onClose} className="absolute top-6 right-6 text-muted-foreground hover:text-white transition-colors bg-card hover:bg-[#1e1e1e] p-2 z-20 brut-card rounded-xl">
          <X className="w-4 h-4" />
        </button>
      </div>
    </div>
